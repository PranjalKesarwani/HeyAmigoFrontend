...
const AllMediaComponent = () => {
    ...
    return (
        <div className={`absolute w-full h-full  top-0 left-0 z-10 p-2  flex flex-col items-center justify-start ${isChecked ? 'bg-slate-800':'bg-slate-300'}`}>
            <i className={`fa-solid fa-circle-xmark text-4xl  absolute top-8 right-8 ${isChecked ? 'text-slate-300':'text-slate-800'}`} role='button' onClick={() => dispatch(setIsAllImages(false))}></i>

            <span className={`${isChecked ? 'text-slate-300':'text-black'}`}>Total images: {dashInfo.allImages.length}</span>

            <div className="w-full   overflow-y-scroll   flex flex-wrap gap-3 items-start justify-start max-[483px]:justify-center  rounded-lg px-20 pt-11 ">

                {

                    dashInfo.allImages.length === 0 ? <>
                        <div className={`w-full h-full flex items-center justify-center text-8xl ${isChecked ? 'text-slate-300':'text-black'}`}><h1>No Images</h1></div>
